...
//Mine
void Perlin3D_float(float3 Coordinate, out float Out)
{
    Out = cnoise(Coordinate);
}
void PerlinFractal3D_float(float3 Coordinate, float Octaves, float Roughness, out float Out)
{
    float fscale = 1.0f;
    float amp = 1.0f;
    float maxamp = 0.0f;
    ...
    for(int i = 0;i<oct;i++)
    {
        float n = cnoise(fscale*Coordinate);
        sum += n*amp;
        maxamp += amp;
