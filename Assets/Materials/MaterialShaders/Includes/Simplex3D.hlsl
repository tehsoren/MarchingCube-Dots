...
// port by https://github.com/keijiro/NoiseShader 
//
...
void SimplexNoise3D_float(float3 Coordinate, out float Out)
{
    Out = SimplexNoiseGrad(Coordinate).w;
}

float SimplexNoiseFractal3D_float(float3 Coordinate, float Octaves, float Roughness)
{
    float fscale = 1.0f;
    float amp = 1.0f;
    float maxamp = 0.0f;
    float sum = 0.0f;
    int oct = clamp(int(Octaves),0,15);
    for(int i = 0;i<oct;i++)
    {
        float n = SimplexNoiseGrad(fscale*Coordinate).w;
        sum += n*amp;
        maxamp += amp;
